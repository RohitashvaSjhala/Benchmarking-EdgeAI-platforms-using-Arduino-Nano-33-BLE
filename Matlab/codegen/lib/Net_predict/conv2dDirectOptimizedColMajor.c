...
static void convolution(const float *inputTensor, float *outputTensor,
                        const float *weightsTensor, const float *biasTensor)
{
  static float inputScratchpadBuffer[2432];
  static bool bufferInitialized;
  int c_inputChannelMiniblockBaseIdxS;
  int fusedInputChannelMiniBlockIdx;
  int fusedInputWidthAndHeightIdx;
  if (!bufferInitialized) {
    memset(&inputScratchpadBuffer[0], 0, 9728U);
  ...
  for (fusedInputChannelMiniBlockIdx = 0; fusedInputChannelMiniBlockIdx < 150;
    ...
    c_inputChannelMiniblockBaseIdxS = (fusedInputWidthAndHeightIdx << 4) + 16;
  ...
  for (fusedInputChannelMiniBlockIdx = 0; fusedInputChannelMiniBlockIdx < 300;
    ...
    convolutionKernel(
        &inputScratchpadBuffer[c_inputChannelMiniblockBaseIdxS << 4],
        &outputTensor[c_inputChannelMiniblockBaseIdxS +
                      fusedInputWidthAndHeightIdx * 2400],
        &weightsTensor[fusedInputWidthAndHeightIdx * 768],
                              ...
                              const float *biasBufferPtr)
{
  ...
  for (kernelWidthIdx = 0; kernelWidthIdx < 3; kernelWidthIdx++) {
    for (inputChannelIdx = 0; inputChannelIdx < 16; inputChannelIdx++) {
      ...
      inputRegister_tmp = weightsIdx_tmp_tmp + inputChannelIdx;
      ...
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 48];
      b_outputRegister += inputRegister * weightsRegister;
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 96];
      ...
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 192];
      e_outputRegister += inputRegister * weightsRegister;
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 240];
      ...
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 336];
      h_outputRegister += inputRegister * weightsRegister;
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 384];
      ...
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 480];
      k_outputRegister += inputRegister * weightsRegister;
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 528];
      ...
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 624];
      n_outputRegister += inputRegister * weightsRegister;
      weightsRegister = weightsBufferPtr[inputRegister_tmp + 672];
    ...
    weightsIdx_tmp_tmp = weightsIdx + 16;
    weightsIdx += 16;
  }
  outputBufferPtr[0] = fmaxf(outputRegister, 0.0F);
  outputBufferPtr[150] = fmaxf(b_outputRegister, 0.0F);
  outputBufferPtr[300] = fmaxf(c_outputRegister, 0.0F);
  outputBufferPtr[450] = fmaxf(d_outputRegister, 0.0F);
  outputBufferPtr[600] = fmaxf(e_outputRegister, 0.0F);
  outputBufferPtr[750] = fmaxf(f_outputRegister, 0.0F);
  outputBufferPtr[900] = fmaxf(g_outputRegister, 0.0F);
  outputBufferPtr[1050] = fmaxf(h_outputRegister, 0.0F);
  outputBufferPtr[1200] = fmaxf(i_outputRegister, 0.0F);
  outputBufferPtr[1350] = fmaxf(j_outputRegister, 0.0F);
  outputBufferPtr[1500] = fmaxf(k_outputRegister, 0.0F);
  outputBufferPtr[1650] = fmaxf(l_outputRegister, 0.0F);
  outputBufferPtr[1800] = fmaxf(m_outputRegister, 0.0F);
  outputBufferPtr[1950] = fmaxf(n_outputRegister, 0.0F);
  outputBufferPtr[2100] = fmaxf(o_outputRegister, 0.0F);
  outputBufferPtr[2250] = fmaxf(p_outputRegister, 0.0F);
}

/*
 * Arguments    : const float X[450]
 *                float Z[4800]
 * Return Type  : void
 */
void conv2dDirectOptimizedColMajor(const float X[450], float Z[4800])
{
  static const float reformattedAndTruncatedWeights[1536] = {0.0637785569F,
                                                             -0.563906431F,
                                                             -0.0958373F,
                                                             0.0F,
                                                             0.0F,
